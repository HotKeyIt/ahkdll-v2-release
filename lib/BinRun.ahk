...
BinRun(pData,cmdLine:="",cmdLineScript:="",ExeToUse:=""){
  ...
  ,h2o:="B29C2D1CA2C24A57BC5E208EA09E162F(){`nPLACEHOLDERB29C2D1CA2C24A57BC5E208EA09E162FVarSetCapacity(dmp,sz:=StrLen(hex)//2,0)`nLoopParse,`%hex`%`nIf (`"`"!=h.=A_LoopField) && !Mod(A_Index,2)`nNumPut(`"0x`" h,&dmp,A_Index/2-1,`"UChar`"),h:=`"`"`nreturn ObjLoad(&dmp,sz)`n}`n"
                                ...
                                if (Script){ ; use pipe to pass script to new executable
                                  If IsObject(cmdLineScript){
                                    ...
                                    Loop % sz:=ObjDump(cmdLineScript,dmp)
                                      If NumPut(NumGet(&dmp,A_Index-1,"UChar"),&val,"UIn64") && DllCall("msvcrt\_vsnwprintf","Str",buf,"ptr",6,"str","`%02X","ptr",&val,"Cdecl")
                                        hex.=buf
                                    While % _hex:=SubStr(Hex,1 + (A_Index-1)*16370,16370)
                                      _s.= "hex" (A_Index=1?":":".") "=`"" _hex "`"`n"
                                    script:=StrReplace(h2o,"PLACEHOLDERB29C2D1CA2C24A57BC5E208EA09E162F",_s) "global A_Argv:=B29C2D1CA2C24A57BC5E208EA09E162F()`n" script
                                  }
                                  ...
                                  if !DllCall("WriteFile","PTR",__PIPE_,"str",chr(0xfeff) script,"UInt",(StrLen(script)+1)*2+4,"UInt*",0,"PTR",0)
