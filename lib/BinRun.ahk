...
BinRun(pData,cmdLine:="",cmdLineScript:="",Hide:=0,ExeToUse:=""){
  ...
  if CreateProcess(0,"`"" UsedExe "`"" A_Space cmdLine (cmdLineScript?A_Space cmdLineScript:""),0,0,0,CREATE_SUSPENDED,0,0,si[],pi[]){
      if (Force32Bit && Wow64GetThreadContext(pi.hThread,ctx[])) || (!Force32Bit && GetThreadContext(pi.hThread,ctx[])){ ;DllCall((Force32Bit?"Wow64":"") "GetThreadContext","PTR",pi.hThread,"PTR", ctx[]){
              ...
              if (pImagebase)
              {
                  if WriteProcessMemory(pi.hProcess, pImagebase, pData, pNtHeader.OptionalHeader.SizeOfHeaders, getvar(NumberOfBytes:=0)){
                      ...
                      if WriteProcessMemory(pi.hProcess, pPebImageBase, pNtHeader.OptionalHeader.ImageBase[""], (Force32Bit?4:A_PtrSize), getvar(NumberOfBytes:=0)){
                          ...
                          if (Force32Bit && Wow64SetThreadContext(pi.hThread, ctx[])) || (!Force32Bit && SetThreadContext(pi.hThread, ctx[])){ ;DllCall((Force32Bit?"Wow64":"") "SetThreadContext","PTR",pi.hThread, "PTR",ctx[]){
...
; VarZ 2.0 by SKAN, 27-Sep-2012. http://www.autohotkey.com/community/viewtopic.php?t=45559
 ...
 VarSetCapacity( D,Final,0 ), RtlMoveMemory(&D, &TD, Final)
