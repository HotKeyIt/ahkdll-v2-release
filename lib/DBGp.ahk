...
DBGp_HandleWindowMessage(hwnd, uMsg, wParam, lParam)
{
	...
	else if (event = FD_CLOSE) ; Connection closed.
	{
        ...
        session.responseQueue.Push("<response><error code=`"999`"/></response>")
...
DBGp_HandleIncomingData(session)
{
    ...
    if (len >= packetLen)  ; We have a complete packet.
    {
        ...
        else if (packetType.Value = "response")
            DBGp_HandleResponsePacket(session, packet)
        else if (packetType.Value = "stream")
            DBGp_HandleStreamPacket(session, packet)
        else if (packetType.Value = "init")
            DBGp_HandleInitPacket(session, packet)
        else
            DBGp_E("unknown packet type: " packetType.Value)
    }
...
DBGp_HandleInitPacket(session, ByRef packet)
{
    ...
    session.IDEKey := DBGp_DecodeXmlEntities(idekey.Value)
    session.Cookie := DBGp_DecodeXmlEntities(cookie.Value)
    session.Thread := thread.Value
    session.File   := DBGp_DecodeFileURI(fileuri.Value)
...
DBGp_E(n)
{
	global DBGp_ErrorLevel
    ; D("E " n)
	DBGp_ErrorLevel := n
	if Type(DBGp_ErrorLevel+0)="Integer"
		return DBGp_ErrorLevel ; Return DBGp error code.
