WinApiDef(def){
	static buf,init:=UnZipRawMemory(LoadResource(handle:=GetModuleHandle(A_DllPath),res:=FindResource(handle,"C974C3B7677A402D93B047DA402587C7",10)),SizeOfResource(handle,res),buf),list:=StrGet(&buf,"UTF-8")
