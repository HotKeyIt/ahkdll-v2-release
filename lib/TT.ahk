...
Class _ToolTipClass{
  ...
  ,TRANSPARENT:="",Color:="",Background:="",icon:=0
    ...
    If !(AUTOPOP INITIAL RESHOW)
      this.SETDELAYTIME()
    else this.SETDELAYTIME(2,AUTOPOP?AUTOPOP:-1),this.SETDELAYTIME(3,INITIAL?INITIAL:-1),this.SETDELAYTIME(1,RESHOW?RESHOW:-1)
    this.fulltext:=text,this.maintext:=RegExReplace(text,"<a\K[^<]*?>",">")
    If OnClick
      ParseLinks:=1
  ...
  ADD(Control,Text:="",uFlags:="",Parent:=""){
    ...
    If (Type(Control)!="Integer")
      Control:=ControlGetHwnd(Control,"ahk_id " (Parent?Parent:this.P.hwnd))
    If uFlags
  ...
  Show(text:="",x:="",y:="",title:="",icon:=0,icon_:=1,defaulticon:=1){
    ...
    this.TRACKPOSITION(x,y),this.TRACKACTIVATE(1)
    ;DllCall("SetWindowPos","PTR",this.hwnd,"PTR",0,"Int",0,"Int",0,"Int",0,"Int",0x1|0x2|0x10|0x8|0x200|0x400)
    ; ControlFocus,,% "ahk_id " this.hwnd
  }
  ...
  GETDELAYTIME(whichtime){
    ;TTDT_RESHOW = 1; TTDT_AUTOPOP = 2; TTDT_INITIAL = 3
     ...
     return DllCall("SendMessage","PTR",this.HWND,"UInt",TTM_GETDELAYTIME,"PTR",whichtime,"PTR",0,"PTR")
  }
  GETMARGIN(pRECT){
     static TTM_GETMARGIN := 0x41b
     Return DllCall("SendMessage","PTR",this.HWND,"UInt",TTM_GETMARGIN,"PTR",0,"PTR",pRECT,"PTR")
  }
  GETMAXTIPWIDTH(wParam:=0,lParam:=0){
  ...
  GETTIPBKCOLOR(wParam:=0,lParam:=0){
     ...
     return DllCall("SendMessage","PTR",this.HWND,"UInt",TTM_GETTIPBKCOLOR,"PTR",0,"PTR",0,"PTR")
  }
  GETTIPTEXTCOLOR(wParam:=0,lParam:=0){
  ...
  GETTOOLCOUNT(wParam:=0,lParam:=0){
  ...
  POP(wParam:=0,lParam:=0){
     ...
     Return DllCall("SendMessage","PTR",this.HWND,"UInt",TTM_POP,"PTR",0,"PTR",0,"PTR")
  }
  POPUP(wParam:=0,lParam:=0){
  ...
  SETDELAYTIME(whichTime:=0,mSec:=-1){
    ;TTDT_AUTOMATIC = 0; TTDT_RESHOW = 1; TTDT_AUTOPOP = 2; TTDT_INITIAL = 3
     static TTM_SETDELAYTIME := 0x400 + 3
     Return DllCall("SendMessage","PTR",this.HWND,"UInt",TTM_SETDELAYTIME,"PTR",whichTime,"PTR",mSec,"PTR")
  }
  SETMARGIN(left:=0,top:=0,right:=0,bottom:=0){
     static TTM_SETMARGIN := 0x41a
    rc:=this.rc,rc.top:=top,rc.left:=left,rc.bottom:=bottom,rc.right:=right
    Return DllCall("SendMessage","PTR",this.HWND,"UInt",TTM_SETMARGIN,"PTR",0,"PTR",rc[],"PTR")
  }
  SETMAXTIPWIDTH(maxwidth:=-1){
     static TTM_SETMAXTIPWIDTH := 0x418
     return DllCall("SendMessage","PTR",this.HWND,"UInt",TTM_SETMAXTIPWIDTH,"PTR",0,"PTR",maxwidth,"PTR")
  }
  SETTIPBKCOLOR(color:=0){
    ...
    Return DllCall("SendMessage","PTR",this.HWND,"UInt",TTM_SETTIPBKCOLOR,"PTR",color,"PTR",0,"PTR")
  }
  SETTIPTEXTCOLOR(color:=0){
  ...
  TRACKPOSITION(x:=0,y:=0){
    Return DllCall("SendMessage","PTR",this.HWND,"UInt",0x412,"PTR",0,"PTR",(x & 0xFFFF)|(y & 0xFFFF)<<16,"PTR")
  }
  UPDATE(){
