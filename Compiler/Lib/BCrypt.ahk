BCrypt(Algorithm, Buffer, Key:=""){
    ...
    if BCryptOpenAlgorithmProvider[getvar(hAlgo:=0), StrUpper(Algorithm), 0, Key?0x00000008:0]
        ...
        return
    pbHashObj:=BufferAlloc(cbHashObj)
    ,Key ? (pbSecret:=BufferAlloc(cbSecret := StrPut(Key, "UTF-8")),StrPut(Key, pbSecret, "UTF-8"),pb:=pbSecret.Ptr,cb:=cbSecret - 1) : (pb:=cb:=0)
    ,aBuffer:=Type(Buffer)!="Buffer" ? (aBuffer:=BufferAlloc(size := StrPut(Buffer, "UTF-8")),StrPut(Buffer, aBuffer, "UTF-8"),size--,aBuffer) : Buffer
    ,hash:=(pbHash:=BufferAlloc(cbHash))  && !BCryptCreateHash[hAlgo, getvar(hHash:=0), pbHashObj, cbHashObj, pb, cb , 0] && !BCryptHashData[hHash, aBuffer, aBuffer.size, 0]	&& !BCryptFinishHash[hHash, pbHash, cbHash, 0] ? BinToHex(pbHash, cbHash) : ""
