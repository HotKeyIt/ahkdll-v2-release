...
Progress(aOptions:="",aSubText:="", aMainText:="", aTitle:="",aFontName:=""){
  ...
  splash.hwnd := CreateWindowEx(exstyle, "AutoHotkeyGUI", aTitle, style, xpos, ypos
  ...
  if (splash.object_height > 0) ;  Progress window
  {
    ...
    if (splash.hwnd_bar := CreateWindowEx(WS_EX_CLIENTEDGE, PROGRESS_CLASS, "", WS_CHILD|WS_VISIBLE|PBS_SMOOTH
      , splash.margin_x, bar_y, control_width, splash.object_height, splash.hwnd, 0, 0, 0))
    {
      ...
      if (bar_color != CLR_DEFAULT)
      {
        ...
        SendMessage_(splash.hwnd_bar, PBM_SETBARCOLOR, 0, bar_color) ;  Set color.
      }
      if (splash.color_bk != CLR_DEFAULT)
        SendMessage_(splash.hwnd_bar, PBM_SETBKCOLOR, 0, splash.color_bk) ;  Set color.
      if (bar_pos_has_been_set) ;  Note that the window is not yet visible at this stage.
        ...
        bar_pos := SendMessage_(splash.hwnd_bar, PBM_GETPOS, 0, 0)
